...
{
	uint32_t src_size, img_size, src_xy, out_size, out_xy, stride, format, bpp, unpack_pattern;
	...
	unpack_pattern = readl(pipe->base + PIPE_SSPP_SRC_UNPACK_PATTERN);

	dprintf(INFO, "MDP continuous splash detected: pipe %s, base: %p, stride: %d, "
		"src: %dx%d (%d,%d), img: %dx%d, out: %dx%d (%d,%d), format: %#x (bpp: %d), unpack_pattern: %#x\n",
